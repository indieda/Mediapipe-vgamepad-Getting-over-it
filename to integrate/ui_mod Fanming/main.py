...
INVERT_X             = False
INVERT_Y             = True
WRIST_SPEED_HISTORY_SEC = 1.0
WRIST_SPEED_SCALE       = 600.0
...
# 右手：实时控制历史与EMA
r_hist = deque()
left_wrist_hist = deque()
right_wrist_hist = deque()
rvx_ema = 0.0
rvy_ema = 0.0
...
with mp_pose.Pose(static_image_mode=False, min_detection_confidence=0.5, model_complexity=1) as pose, \
    ...
    while True:
        ...
        if lw_x is not None and lw_y is not None:
            left_wrist_hist.append((now_t, lw_x, lw_y))
            while left_wrist_hist and (now_t - left_wrist_hist[0][0]) > WRIST_SPEED_HISTORY_SEC:
                left_wrist_hist.popleft()
        if rw_x is not None and rw_y is not None:
            right_wrist_hist.append((now_t, rw_x, rw_y))
            while right_wrist_hist and (now_t - right_wrist_hist[0][0]) > WRIST_SPEED_HISTORY_SEC:
                right_wrist_hist.popleft()

        current_line_metrics["left_wrist_speed"] = min(200.0, compute_speed_from_hist(left_wrist_hist) * WRIST_SPEED_SCALE)
        current_line_metrics["right_wrist_speed"] = min(200.0, compute_speed_from_hist(right_wrist_hist) * WRIST_SPEED_SCALE)
